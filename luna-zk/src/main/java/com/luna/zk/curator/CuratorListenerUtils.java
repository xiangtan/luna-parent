...
public class CuratorListenerUtils {
	private static final String CONNECTING_STRING = Constant.ZK_ADDRESS;
	...
	private static CuratorFramework clientOne() {
		...
		CuratorFramework client = CuratorFrameworkFactory.newClient(CONNECTING_STRING, retryPolicy);
		client.start();
		return client;
	}
	...
	private static CuratorFramework clientTwo() {
		...
		CuratorFramework client = CuratorFrameworkFactory.builder().aclProvider(aclProvider).authorization(scheme, auth)
				.connectionTimeoutMs(connectionTimeoutMs).connectString(CONNECTING_STRING).namespace(namespace)
